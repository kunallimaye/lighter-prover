...
# render_startup <image> <svalues> <host_label> <run_id> [<cal_mode>] [<cal_l4>] -> path
...
# <cal_l4> (issue #102 Phase C): "1" adds one --l2-fold tree --l4-check
# probe so MERGE_S/L4_WALL are measured per shape; default "0" = off.
render_startup() {
...
provision_one_vm() {
  ...
  local startup_path
  startup_path="$(render_startup "${image}" "${svalues}" "${mt}" "${run_id}" "${CAL_MODE:-0}" "${CAL_L4:-0}")"
