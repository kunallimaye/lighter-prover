    ...
    match cli.role {
        ...
        Role::TreeNode { level, node_idx, radix } => {
            ...
            if let Some(pubsub) = PubSubClient::new("lighter-prod") {
                ...
                if radix == 16 {
                    info!("Paging Pub/Sub HTTP REST network stream on subscription {sub_name} for 16 child proofs ({} through {})...", 16 * node_idx, 16 * node_idx + 15);
                    for c in 0..16 {
                        pubsub.pull_message(&sub_name, expected_role, 16 * node_idx + c);
                    }
                } else {
...
>>>>>>> a81c80b (Unmock distributed proving reduction trees & root coordinator with HTTP Pub/Sub backplane and timestamp-grouped telemetry)
                "elapsed_ms": start.elapsed().as_millis(),
                "status": "OK"
            });
            println!("{}", report);
