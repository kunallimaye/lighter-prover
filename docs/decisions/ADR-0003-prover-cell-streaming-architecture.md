...
**Scaling (Path C — #82):** a pre-L5 block-proof merge node — two conditional same-shape cyclic verifications on the L5 `Batch`+`SegmentInfo` PI surface, `conditionally_merge_consecutive` glue, and the on-chain-ops keccak **start-digest stitch** — **builds at 2^14 (12,503 pre-padding gates) and pads into the L5 2^15 self-shape at ~38% gate utilization** (calibration-anchored probe; control rows matched #10/#64 measurements). Merge step ≈ today's ~0.94 s fold ⇒ a log-depth tree gives ~log₂(N)·0.94 s critical path and no arity ceiling. **Landed (build-validated, #82):** the merge circuit is implemented as `BatchMergeCircuit` in `circuit/src/recursion/batch_merge_constraints.rs` (sibling of the L2 merge precedent `block_tx_chain_merge_constraints.rs`); it builds into the L5 cyclic circuit's exact self-shape — the driver asserts `merge.common == l5.common` (degree 2^15, 1496 PIs) at build time, and an element-wise A/B check confirms the tree root's `Batch`+`SegmentInfo` PI surface equals the L5 serial fold's (trailing VK PIs differ by construction: L5 leaf VK vs merge VK). The non-associativity concern is addressed by the keccak-chain `SegmentInfo` **start-digest stitch** (`connect_segments`, escape hatch iii), mirroring the L6 form at `wrapper_circuit.rs:196-200`. The host-side merge mirror (`Batch::merge_consecutive` / `SegmentInfo::stitch`, unit-tested in `batch.rs`) validates the same field/keccak-chain semantics off-circuit, and the bench driver's `--l5-fold tree` path wires the log-depth fold. The **live timed prove** (≥4-leaf tree on the AMD EPYC 7B13 baseline, confirming ≈0.94 s/step and the log₂(N)·0.94 s critical path) is split out as a documented follow-up run requiring dedicated hardware + long wall-clock (#90).
...
**Numbering rationale**: this is an *in-place* amendment to ADR-0003, **NOT a new ADR**. ADR-0002 is still reserved for #10's deliverable and the duplicate-ADR-0001 collision (#68) is unresolved; adding a new number before the numbering is healed compounds the mess. (Note on the ADR-0005 number: the merged tree's ADR-0005 is the L6-inner-wrapper/KZG-sidecar doc; the distributed-prover *conductor* design — which this amendment references — was renumbered to **ADR-0006** when PR #115 merged, resolving the would-be duplicate-0005 collision.) As with the 2026-06-13 cross-cell amendment above, the original §D1–§D8 / Consequences / Risks body stays in place; this block supersedes specific §D7 sentences by quoting them verbatim and marking them, so the change history stays legible.
