...
Running several blocks per coordinator at once is a PROMISING efficiency multiplier that would shrink the pool — but it is a BUILD-TIME HYPOTHESIS WITH NO CURRENT MEASUREMENT: no per-coordinator utilization profile has been captured, so whether the box has slack to absorb concurrent blocks is unmeasured. The intuition is that L4's all-core work might interleave with other blocks' quieter phases, but it is NOT YET PROVEN — it depends on L4 bursts interleaving rather than colliding, and concurrency scaled poorly at L2/L5 (a real, supported caveat — see §6.1 streaming verdict and the #112 scheduling spike). Capturing that utilization profile is itself the build-time measurement deferred to the coordinator build (#113). The pool (proven) is the primary lever; per-coordinator concurrency (unmeasured hypothesis, to validate) is a bonus that, if it holds, tunes the count. Tracked in #113.

> **Correction (2026-06-14).** An earlier version of this sub-paragraph cited a coordinator-utilization measurement — *"a utilization measurement (dedicated c4a-highcpu-64, tip `1211ffc`; `calibration/coordinator-utilization-c4a-highcpu-64.json`) found the coordinator workload leaves substantial CPU headroom — the fold leaves the box ~⅔ idle (no core pinned) and L4 is mostly single-threaded serial work with only a ~1s all-core burst"* — that **never existed**. No such run, BENCH-LEDGER row, spike report, or calibration file exists in git history; the file path and the numbers were fabricated (see the forensic trace on **#137**, "World B"). The retracted measurement is removed; the nonexistent file path and the misattribution are removed; and the per-coordinator-concurrency lever is correctly reframed above as an **unmeasured hypothesis**, not a measured result. The surrounding §6.1 thesis (L4 dominance, L4-streaming structurally blocked, the circuit-surgery trigger) is **unaffected** — it stands on the real streaming spike and the calibration ladder, independent of the retracted citation.
