...
2.  **Elastic Volatility Burst (40%+ Spot Pricing)**: Any further market volume spikes above baseload (the remaining 40%+ capacity up to +4 blocks/sec) are absorbed dynamically via **`t2d-standard-60` AMD Milan Proving Pods utilizing Spot pricing**. Symmetrically harvesting any-region global spot capacity collapses burst compute expenditure by > 62% per socket.
