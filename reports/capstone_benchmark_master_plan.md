...
## Goal Description
Jetski will directly orchestrate and execute the comprehensive unmocked comparative STARK proving study in parallel across all configured virtual machines under unified partition `benchmark-id-VM-2026-06-25_10-08-29-1`. 
...
> [!TIP]
> **Sentinel Checkpoint Cache**: Alongside verifying GCS (`gs://.../bench_summary.json`), Jetski maintains a local sentinel file `reports/.checkpoint_benchmark-id-VM-2026-06-25_10-08-29-1.json`. Interrupted sweeps resume instantly reading local state.
...
```bash
export BENCHMARK_ID="benchmark-id-VM-2026-06-25_10-08-29-1"
```
...
2. **Canary Execution (`JOBS=1`, `txs=10`)**: Run a single 1-concurrency canary test across releases (`v0.0.1`, `v0.0.2`). If a container rejects CLI arguments or crashes upon boot, abort immediately so alternatives can be configured before committing full resources.
...
1. **Local Sentinel Check**: Check `reports/.checkpoint_benchmark-id-VM-2026-06-25_10-08-29-1.json` and GCS. If marked `DONE`, skip instantly (`[RESUME] Job <j> already recorded — skipping`).
...
* **Skipped**: Excluded per user request (VMs only).

### Step 5: Unified Metric Extraction
Once all parallel VM workloads complete (or confirm 100% checkpoint existence), directly run:
```bash
python3 infra-as-code/scripts/extract_gcs_metrics.py \
  --gcs-prefix="gs://kunal-scratch-tfstate/benchmark-reports/benchmark-id-VM-2026-06-25_10-08-29-1" \
  --output-json="reports/capstone_benchmark-id-VM-2026-06-25_10-08-29-1.json" \
  --output-csv="reports/capstone_benchmark-id-VM-2026-06-25_10-08-29-1.csv"
```

### Step 6: Google Sheets Ingestion
Confirm automated creation and table ingestion (featuring the renamed **Concurrent Jobs or Blocks** column) into Google Spreadsheet ID `1z8bIeeKaEnXP6UZW52pGLll0XrwjoLS0aBJOvs1qqd0` under sheet tab `2026-06-25_10-08-29-1`.
