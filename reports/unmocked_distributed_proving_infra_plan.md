...
1.  **Layer 1 - Pod Scaling (KEDA --> K8s Deployment)**: KEDA monitors Stackdriver Pub/Sub metric `num_undelivered_messages`. When 1 block arrives (125 unACKed chunks), KEDA scales the Kubernetes Deployment from 0 to 1 Proving Pod Unit (consisting of 6 container pods requesting 240 cores total).
2.  **Layer 2 - Node Scaling (Kube-Scheduler --> Cluster Autoscaler)**: Because the spot node pool is initially at `node_count = 0`, the 6 new pods transition immediately to status `Pending (Unschedulable: Insufficient CPU)`. The GKE Cluster Autoscaler detects these pending pods, inspects their `requests.cpu: "64"` and `nodeSelector: c4a`, and issues a Compute Engine API call provisioning **exactly 4 x `c4a-highcpu-64` Spot VMs** (~35s boot). Once booted, Kubelet binds the 6 pods to physical host core IDs (`cpuset.cpus = 0-63`).
3.  **Autonomous Scale-To-Zero Billing Guarantee (*What happens when input load stops?*)**: Per user review (*“If we boot infra with 2 pods, would it scale back to zero if load stops?”*), **YES! 100% Absolutely.** As soon as input DEX transactions cease and workers issue gRPC ACKs for completed validium blocks, Pub/Sub metric `num_undelivered_messages` drops to `0`. Within 30 seconds (`cooldownPeriod: 30`), KEDA scales pod replicas down to `0`. With 0 pods running, the GKE Cluster Autoscaler detects 100% node idleness. After exactly 10 minutes (`--scale-down-unneeded-time=10m`), the Cluster Autoscaler physically terminates all 8 GCP Spot instances (`compute.instances.delete`), guaranteeing 0.00 standby billing burn!
