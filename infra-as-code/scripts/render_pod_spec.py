...
def main():
  ...
  parser.add_argument("--radix", type=int, default=0, help="Reduction tree radix (2 or 16)")
  ...
  radix = args.radix if args.radix else int(defaults.get("tree_radix", 2))
  tx_count = 500

  levels = math.ceil(math.log(tx_count, radix)) if radix > 1 else 9
  ...
  agg_replicas = levels * args.blocks
...
spec:
  ...
  template:
    ...
    spec:
      ...
      - name: aggregator
        image: us-docker.pkg.dev/lighter-prover/zkp-prover:multiarch
        command: ["prover-node", "tree-node", "--radix", "{radix}"]
        resources:
          limits:
            cpu: "{agg_cpu}"
            memory: "{agg_mem}"
...
"""
  ...
  print(f"[OK] Dynamically rendered K8s Proving Pod manifest to {args.output} (arch={arch}, blocks={args.blocks}, leaf_chunk={leaf_chunk}, radix={radix}, reduction_levels={levels})")
