...
# 0p2pct-merge-*), exactly like the chunk/results planes, instead of the generic
...
coordinator_command = [
  ...
  "--proof-mount-path", "/mnt/proof-store",
  "--fold-distributed",
  "--merge-task-topic", "lighter-prover-scale-0p2pct-merge-task",
  "--merge-result-subscription", "lighter-prover-scale-0p2pct-merge-result-sub",
  "--native-merge-plane",
  "--tx-per-proof", "9",
  "--poll-interval-s", "2",
]
...
# block's wide level-1 actually fan out (its first merge level has the most
# pairs). Sized SEPARATELY from cells + coordinators (ADR-0006: never summed).
# Same full-core c4a/Axion shape + REAL bench image as the coordinator, run in
# --mode fold-worker. NO safe-to-evict/PDB by design: an evicted mid-merge task
# is redelivered to another worker (the #198 at-least-once contract).
enable_fold_workers        = true
fold_worker_replicas       = 4 # a handful so a wide level-1 fans out; scale the fold by worker count (#198)
...
# LIGHTER_PROOF_BUCKET env vars by terraform (no literal tokens here).
# --native-merge-plane selects the native manual-ack streaming-pull client
# (#205). --tx-per-proof 9 matches the cell leaf granularity.
fold_worker_command = [
  "/usr/local/bin/prover", "--mode", "fold-worker",
  "--merge-task-subscription", "lighter-prover-scale-0p2pct-merge-task-sub",
  "--merge-result-topic", "lighter-prover-scale-0p2pct-merge-result",
  "--proof-mount-path", "/mnt/proof-store",
  "--native-merge-plane",
  "--tx-per-proof", "9",
  "--poll-interval-s", "2",
...
merge_task_topic          = "lighter-prover-scale-0p2pct-merge-task"
merge_task_subscription   = "lighter-prover-scale-0p2pct-merge-task-sub"
merge_result_topic        = "lighter-prover-scale-0p2pct-merge-result"
merge_result_subscription = "lighter-prover-scale-0p2pct-merge-result-sub"
